...
import { EnumSchema, VariantDefinition } from './schemas/enum';
...
export declare const s: {
     ...
     */
    nonFungibleLocalId: () => NonFungibleLocalIdSchema;
    /**
     ...
     */
    internalAddress: () => InternalAddressSchema;
    /**
     ...
     * ```
     */
    struct: <T extends StructDefinition>(definition: T) => StructSchema<T, false>;
    /**
     ...
     * ```
     */
    structNullable: <T extends StructDefinition>(definition: T) => StructSchema<T, true>;
    /**
     ...
     * ```
     */
    tuple: <const T extends SborSchema<unknown>[]>(schemas: T) => OrderedTupleSchema<T>;
    /**
     ...
     * ```
     */
    array: <T>(schema: SborSchema<T>) => ArraySchema<SborSchema<T>>;
    /**
     ...
     * }
     */
    enum: <const VARIANT_DEF extends VariantDefinition<S>[], S extends StructSchema<D, B> | OrderedTupleSchema<T>, T extends TupleSchema, D extends StructDefinition, B extends boolean>(variants: VARIANT_DEF) => EnumSchema<VARIANT_DEF>;
    /**
     ...
     */
    option: <T>(schema: SborSchema<T>) => OptionSchema<SborSchema<T>>;
    /**
     * A schema for a Map
     ...
     */
    map: <K, V>(definition: MapDefinition<K, V>) => MapSchema<K, V>;
};
export declare namespace s {
     ...
     * ```
     */
    type infer<T extends SborSchema<any>> = T extends SborSchema<infer R> ? R : never;
}
