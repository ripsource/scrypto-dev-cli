...
impl Network {
    pub fn from_package_address(package_address: &str) -> Result<Self, Box<dyn Error>> {
        if package_address.starts_with("package_rdx") {
            Ok(Network::Mainnet)
        } else if package_address.starts_with("package_tdx_2") {
            Ok(Network::Stokenet)
        } else {
            Err(format!("Invalid package address format: {}", package_address).into())
        }
    }
...
pub fn get_schemas(package_address: &str) -> Result<Vec<Schema>, Box<dyn Error>> {
    let network = Network::from_package_address(package_address)?;
    let base_url = network.gateway_url();

    let payload = json!({ "address": package_address });
    let response: SchemasResponse = Client::new()
        .post(&format!("{}/state/entity/page/schemas", base_url))
...
pub fn get_blueprint_definitions(
    ...
    let url = format!("{}/state/package/page/blueprints", base_url);
...
mod tests {
    ...
    fn test_get_schema() {
        ...
        let schemas = get_schemas(package_address).expect("Failed to get schemas");
    ...
    fn test_get_blueprints_and_schemas() {
        let package_address = "package_rdx1ph3l366k7kq8mg8pzs5d0c855whtqtxkxnlxf2yzxvlelphztlqn05";
        let blueprints_and_schemas = get_blueprints_and_corresponding_schemas(package_address)
            .expect("Failed to get blueprints and schemas");
        println!("Blueprints with Schemas: {:#?}", blueprints_and_schemas);
    }

    // radix stokenet network example package address
    // package_tdx_2_1pkgxxxxxxxxxfaucetxxxxxxxxx000034355863xxxxxxxxx3heqcz
}
