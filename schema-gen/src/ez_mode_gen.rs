...
impl RegistryEntry {
    ...
    /// Returns the struct reference name (with "Struct" suffix for non-inline types)
    pub fn struct_ref_name(&self, registry: &SchemaRegistry) -> String {
        if is_inline(self) {
            self.render(registry)
        } else {
            format!("{}Struct", self.unique_var_name(registry))
        }
    }
    ...
    pub fn render(&self, registry: &SchemaRegistry) -> String {
        match &self.schema_kind {
            ...
            SborEzModeSchemaKind::Map {
                ...
                format!(
                    "s.map({{ key: {}, value: {} }})",
                    key_entry.struct_ref_name(registry),
                    value_entry.struct_ref_name(registry)
                )
            }
            ...
            SborEzModeSchemaKind::Array { element_type } => {
                ...
                format!(
                    "s.array({})",
                    entry.struct_ref_name(registry)
                )
            }
            ...
            SborEzModeSchemaKind::Struct { fields } => {
                let field_entries: Vec<String> = fields
                    ...
                    .map(|(name, field_type)| {
                        let entry = &registry.entries[*field_type as usize];
                        format!("{}: {}", name, entry.struct_ref_name(registry))
                    })
                    .collect();
                format!("s.struct({{\n  {}\n}})", field_entries.join(",\n  "))
            }
...
impl SchemaRegistry {
    ...
    pub fn render(&self, package_address: &str, module: bool) -> String {
        ...
        for i in sorted_indices {
            ...
            let struct_name = entry.unique_var_name(self);
            let const_name = format!("{}Struct", struct_name);
            
            // Add the type export
            output.push_str(&format!(
                "export type {} = s.infer<typeof {}>;\n\n",
                struct_name,
                const_name
            ));
            
            // Add the const export
            output.push_str(&format!(
                "export const {} = {};\n\n",
                const_name,
                entry.render(self)
