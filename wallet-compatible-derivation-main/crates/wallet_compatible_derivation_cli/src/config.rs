...
/// Configuration for generating a new random mnemonic.
#[derive(Debug, Args)]
pub(crate) struct GenerateConfig {
    /// Also generate and display account details for the new mnemonic
    #[arg(short = 'a', long = "accounts", help = "Also generate and display account details for the new mnemonic")]
    pub(crate) generate_accounts: bool,

    /// The Network to use for account generation (if --accounts is specified)
    #[arg(short = 'n', long = "network", help = "The ID of the Radix Network for account generation.", value_parser = NetworkID::from_str, default_value_t = NetworkID::Mainnet)]
    pub(crate) network: NetworkID,

    /// The start account index (if --accounts is specified)
    #[arg(
        short = 's',
        long = "start",
        help = "The start account index to derive the first account at.",
    ...
    /// The number of accounts to derive (if --accounts is specified)
    #[arg(
        short = 'c',
        long = "count",
        help = "The number of accounts to derive, starting at `index`. Max 255.",
    ...
    /// An optional BIP-39 passphrase (if --accounts is specified)
    #[arg(short = 'p', long = "passphrase", help = "Advanced: An optional BIP-39 passphrase for account generation.", default_value_t = String::new())]
    pub(crate) passphrase: String,
}
